    ...
    for (int ax = 0; ax < 3; ++ax)
    {
        ...
        for (i = 0; i <= grid.siz[axis0]; ++i)
        {
            for (j = 0; j <= grid.siz[axis1]; ++j)
            {
                ...
                {
                    pi[axis] = std::min(std::max(0, (int)floor((p[axis] - grid.bbox.min[axis]) / grid.voxel[axis])),
                                        grid.siz[axis]);
                    pi[axis0] = i;
                    pi[axis1] = j;
...
bool Intersection(/*TriMeshType & m, */
                  Plane3<ScalarType> pl, EdgeMeshType &em, double &ave_length, IndexingType *grid,
...
/*@{*/
/**
    Basic Function computing the intersection between  a trimesh and a plane. It returns an EdgeMesh without needing
anything else.
        Note: This version always returns a segment for each triangle of the mesh which intersects with the plane. In
other
        words there are 2*n vertices where n is the number of segments fo the mesh. You can run vcg::edge:Unify to unify
        the vertices closer that a given value epsilon. Note that, due to subtraction error during triangle plane
intersection,
...
void IntersectionBallMesh(TriMeshType &m, const vcg::Sphere3<ScalarType> &ball, TriMeshType &res, float tol = 0)
{
    ...
    if (tol == 0)
        tol = M_PI * ball.Radius() * ball.Radius() / 100000;
    ...
    while (i < res.fn)
    {
        ...
        if (IntersectionSphereTriangle<ScalarType>(ball, res.face[i], witness, &info) && !allIn)
        {
            ...
            {
                // split the face res.face[i] in four, add the four new faces to the mesh and delete the face
                // res.face[i]
