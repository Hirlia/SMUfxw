...
struct LoopWeight
{
    ...
    {
        return (k > 3) ?
                 (5.0 / 8.0 - std::pow((3.0 / 8.0 + std::cos(2.0 * M_PI / SCALAR_TYPE(k)) / 4.0), 2)) / SCALAR_TYPE(k) :
                 3.0 / 16.0;
    }
...
struct RegularLoopWeight
{
    inline SCALAR_TYPE beta(int k)
    {
        static SCALAR_TYPE bkPolar[] = { .32517, .49954, .59549, .625, .63873, .64643, .65127, .67358, .68678, .69908 };
    ...
    {
        static SCALAR_TYPE bkPolar[] = { .15658, .25029, .34547, .375, .38877, .39644, .40132, .42198, .43423, .44579 };

        return (k <= 12) ? bkPolar[k - 3] : LoopWeight<SCALAR_TYPE>().incidentIrregular(k);
    }
    inline SCALAR_TYPE opposite(int k)
    {
        static SCALAR_TYPE bkPolar[] = { .14427, .12524, .11182, .125, .14771, .1768, .21092, .20354, .20505, .19828 };
...
struct ContinuityLoopWeight
{
    inline SCALAR_TYPE beta(int k)
    {
        static SCALAR_TYPE bkPolar[] = { .32517, .50033, .59464, .625, .63903, .67821, .6866, .69248, .69678, .70014 };
    ...
    inline SCALAR_TYPE incidentIrregular(int k)
    {
        static SCALAR_TYPE bkPolar[] = { .15658, .26721, .33539, .375, .36909, .25579, .2521, .24926, .24706, .2452 };

        return (k <= 12) ? bkPolar[k - 3] : LoopWeight<SCALAR_TYPE>().incidentIrregular(k);
    }
    inline SCALAR_TYPE opposite(int k)
    {
        static SCALAR_TYPE bkPolar[] = { .14427, .12495, .11252, .125, .14673, .16074, .18939, .2222, .25894, .29934 };
...
struct LS3Projection
{
    ...
    inline LS3Projection(Scalar beta = 1.0) : beta(beta)
    {
        reset();
    }
    inline void reset()
    ...
    void project(typename MESH_TYPE::VertexType &v) const
    {
        ...
        LScalar aux4 =
          beta * LScalar(0.5) * (sumDotPN - invSumW * sumP.dot(sumN)) / (sumDotPP - invSumW * vcg::SquaredNorm(sumP));
...
template <class MESH_TYPE, class METHOD_TYPE = Centroid<MESH_TYPE>,
          class WEIGHT_TYPE = LoopWeight<typename MESH_TYPE::ScalarType> >
struct OddPointLoopGeneric
  : public std::unary_function<face::Pos<typename MESH_TYPE::FaceType>, typename MESH_TYPE::VertexType>
{
    ...
    void operator()(typename MESH_TYPE::VertexType &nv, face::Pos<typename MESH_TYPE::FaceType> ep)
    {
        ...
        else
        {
            ...
            if (valence && ((*valence)[l] == 6 || (*valence)[r] == 6))
            {
                ...
                proj.addVertex(*l,
                               ((*valence)[l] == 6) ? weight.incidentRegular(extra) : weight.incidentIrregular(extra));
                proj.addVertex(*r,
                               ((*valence)[r] == 6) ? weight.incidentRegular(extra) : weight.incidentIrregular(extra));
    ...
    Color4<typename MESH_TYPE::ScalarType> WedgeInterp(Color4<typename MESH_TYPE::ScalarType> &c0,
  ...
  : public std::unary_function<face::Pos<typename MESH_TYPE::FaceType>, typename MESH_TYPE::VertexType>
{
    ...
    void operator()(typename MESH_TYPE::VertexType &nv, face::Pos<typename MESH_TYPE::FaceType> ep)
    {
        ...
        if (he.IsBorder())
        {  //	Border rule
            // consider valence of borders as if they are half+1 of an inner vertex. not perfect, but better than
            // nothing.
            if (valence)
            {
                k = 0;
...
bool RefineOddEven(MESH_TYPE &m, ODD_VERT odd, EVEN_VERT even, float length, bool RefineSelected = false,
                   CallBackPos *cbOdd = 0, CallBackPos *cbEven = 0)
{
...
bool RefineOddEvenE(MESH_TYPE &m, ODD_VERT odd, EVEN_VERT even, PREDICATE edgePred, bool RefineSelected = false,
                    CallBackPos *cbOdd = 0, CallBackPos *cbEven = 0)
{
    typedef typename MESH_TYPE::template PerVertexAttributeHandle<int> ValenceAttr;
    ...
    for (fi = m.face.begin(); fi != m.face.end(); fi++)
        if (!(*fi).IsD() && (!RefineSelected || (*fi).IsS()))
        {  // itero facce
            for (int i = 0; i < 3; i++)
            {  // itero vert
