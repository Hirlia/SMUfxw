...
class Inertia
{
  ...
  private:
    ...
    inline ScalarType SQR(ScalarType &x) const
    {
        return x * x;
    }
    inline ScalarType CUBE(ScalarType &x) const
    {
        return x * x * x;
    }
  ...
  public:
    ...
    void CompFaceIntegrals(FaceType &f)
    {
        ...
        Fbb = k1 * Pbb;
        Fcc =
          k3 * (SQR(n[A]) * Paa + 2 * n[A] * n[B] * Pab + SQR(n[B]) * Pbb + w * (2 * (n[A] * Pa + n[B] * Pb) + w * P1));

        Faaa = k1 * Paaa;
        Fbbb = k1 * Pbbb;
        Fccc = -k4 * (CUBE(n[A]) * Paaa + 3 * SQR(n[A]) * n[B] * Paab + 3 * n[A] * SQR(n[B]) * Pabb +
                      CUBE(n[B]) * Pbbb + 3 * w * (SQR(n[A]) * Paa + 2 * n[A] * n[B] * Pab + SQR(n[B]) * Pbb) +
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                ...
                */
                ...
                DC += tmp * 0.5;
                // 		DC*=fabs(A.Determinant());	// the determinant of A is the jacobian of the change of variables
                // A*x+delta
