...
{
  public:
    Output_File(const std::string& file) : _file()
    {
...
class ExporterIDTF
{
  public:
    ...
    {
        static const char* dae_error_msg[] = { "No errors" };
    ...
    static void restoreConvertedTextures(SaveMeshType& mesh_with_textures_to_be_restored,
    ...
    static int Save(SaveMeshType& m, const char* file, const int mask)
    {
        ...
        if ((mask & Mask::IOM_WEDGTEXCOORD) | (mask & vcg::tri::io::Mask::IOM_VERTCOLOR) |
            (mask & vcg::tri::io::Mask::IOM_FACECOLOR))
        {
        ...
        if (mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD)
            idtf.write(3, "MODEL_TEXTURE_COORD_COUNT " + TextUtility::nmbToStr(m.face.size() * 3));
        else
            idtf.write(3, "MODEL_TEXTURE_COORD_COUNT 0");
        ...
        for (ConstFaceIterator fit = m.face.begin(); fit != m.face.end(); ++fit)
        {
            idtf.write(4, TextUtility::nmbToStr(nn) + " " + TextUtility::nmbToStr(nn + 1) + " " +
        ...
        if (mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD)
        {
            ...
            for (unsigned int ii = 0; ii < m.face.size(); ++ii)
            {
                ...
                idtf.write(5, "TEXTURE_LAYER 0 TEX_COORD: " + TextUtility::nmbToStr(ii * 3) + " " +
                                TextUtility::nmbToStr(ii * 3 + 1) + " " + TextUtility::nmbToStr(ii * 3 + 2));
                idtf.write(4, "}");
            }
            idtf.write(3, "}");
        }
        ...
        for (ConstVertexIterator vit = m.vert.begin(); vit != m.vert.end(); ++vit)
        {
            ...
            idtf.write(4, TextUtility::nmbToStr(-tmp.X()) + " " + TextUtility::nmbToStr(tmp.Z()) + " " +
        ...
        for (FaceIterator fitn = m.face.begin(); fitn != m.face.end(); ++fitn)
        {
            ...
            {
                fitn->N().Normalize();
                idtf.write(4, TextUtility::nmbToStr(-fitn->N().X()) + " " + TextUtility::nmbToStr(fitn->N().Z()) + " " +
            ...
            for (FaceIterator vit = m.face.begin(); vit != m.face.end(); ++vit)
            {
                for (unsigned int ii = 0; ii < 3; ++ii)
                {
                    ...
                    idtf.write(4, TextUtility::nmbToStr(float(cc.X()) / 255.0f) + " " +
                                    ...
                                    TextUtility::nmbToStr(float(cc.Z()) / 255.0f) + " " +
                                    TextUtility::nmbToStr(float(cc.W()) / 255.0f));
                }
            }
            idtf.write(3, "}");
        }
        ...
        // if (!(mask & vcg::tri::io::Mask::IOM_WEDGTEXCOORD) & (mask & vcg::tri::io::Mask::IOM_VERTCOLOR) | (mask &
