...
double Branch::getNodeLengthEulerDist(int idx) const
{
    ...
    {
        length = GlobalFun::computeEulerDist(curve[curve.size() - 1], curve[curve.size() - 2]);
    }
}
...
double Branch::getNodeLengthEulerDistSquare(int idx) const
{
    ...
    {
        length = GlobalFun::computeEulerDistSquare(curve[curve.size() - 1], curve[curve.size() - 2]);
    }
}
...
bool Branch::isVirtualTailHealthy() const
{
    ...
    if (angle < save_virtual_angle || (tail_length < save_virtual_dist && angle < bad_virtual_angle))
    {
        return true;
    }
    else
...
void Branch::rememberVirtualHead()
{
    ...
    double old_length = GlobalFun::computeEulerDistSquare(old_virtual_head, real_head);
    double new_length = GlobalFun::computeEulerDistSquare(new_virtual_head, real_head);
...
void Branch::rememberVirtualTail()
{
    ...
    double old_length = GlobalFun::computeEulerDistSquare(old_virtual_tail, real_tail);
    double new_length = GlobalFun::computeEulerDistSquare(new_virtual_tail, real_tail);
...
{
    return (curve[curve.size() - 1].P() - curve[curve.size() - 2].P()).Normalize();
}
...
Point3f Branch::getTrueTailDirection()
{
    ...
    {
        return (curve[curve.size() - 2].P() - curve[curve.size() - 3].P()).Normalize();
    }
}
