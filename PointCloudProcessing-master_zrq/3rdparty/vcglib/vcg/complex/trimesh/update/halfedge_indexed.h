...
class UpdateHalfEdges
{
  public:
    ...
    {
        VertexPairEdgePtr(VertexPointer _v0, VertexPointer _v1, HEdgePointer _ep) : v0(_v0), v1(_v1), ep(_ep)
        {
            if (v0 > v1)
                std::swap(v0, v1);
        }
        const bool operator<(const VertexPairEdgePtr &o) const
        {
    ...
    /**
                build a half-edge data structure from an indexed data structure. Note that the half-edges are allocated
       here for the first time.
                If you have a mesh where there are already edges, they will be removed and the data lost, so do not use
       this function
    ...
    static void FromIndexed(MeshType &m)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            ...
            {
                assert((*fi).VN() > 2);
                if (flagVisited[*fi].empty())
                {
                    flagVisited[*fi].resize((*fi).VN());
                }
                ...
                for (int i = 0; i < (*fi).VN(); ++i, ++ei)
                {
                    ...
                    if (HasVHAdjacency(m))
                        (*ei).HVp()->VHp() = &(*ei);
                    all.push_back(VertexPairEdgePtr((*fi).V(i), (*fi).V((*fi).Next(i)),
                                                    &(*ei)));  // it will be used to link the hedges
...
**/
    static bool CheckConsistency_FHp(MeshType &m)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); ++fi)
            if (!(*fi).IsD())
            {
                if ((*fi).FHp() < &(*m.hedge.begin()))
                    return false;
                if ((*fi).FHp() > &(m.hedge.back()))
                    return false;
            }
        return true;
    }
...
*/
    static void AddHEdge(MeshType &m, HEdgeType *e0, HEdgeType *e1)
    {
        ...
        {  // does not have pointer to previous, it must be computed
            ep = e0;
            do
            {
                if (ep->HNp() == e0)
                ...
                if (!hasHEF || (hasHEF && (*ei).HFp() != NULL))  // if it has a pointer to the face it is
                    ...
                    if (!hV[(*ei)])  // it has not be visited yet
                    {
                        ...
                        for (unsigned int i = 0; i < vpts.size(); ++i)
                            fp->V(i) = vpts[i];  // set the pointer from face to vertex
