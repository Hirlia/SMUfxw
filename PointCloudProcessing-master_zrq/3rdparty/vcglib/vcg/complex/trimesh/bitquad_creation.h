  ...
  public:
    ...
    static void selectBestDiag(FaceType *fi)
    {
        ...
        for (int k = 0; k < 3; k++)
        {
            ...
            if (fi->FFp(k) == fi)
                continue;  // never make a border faux
    ...
    /**
     * This function split a face along the specified border edge it does not compute any property of the new vertex. It
     * only do the topological work.
    ...
    static std::pair<typename MeshType::FaceType *, typename MeshType::VertexType *>
    FaceSplitBorderEdge(MeshType &m, typename MeshType::FaceType &f, int edge, typename MeshType::FaceType *newFace,
                        typename MeshType::VertexType *newVert)
    {
        ...
        if (newFace == 0)
            newFace = &*tri::Allocator<MeshType>::AddFaces(m, 1);
        if (newVert == 0)
        {
        ...
        // qDebug("NewFace %i %i
        // %i",tri::Index(m,newFace->V(0)),tri::Index(m,newFace->V(1)),tri::Index(m,newFace->V(2)));
    ...
    {
        if (m.fn % 2 == 0)
            return false;  // it's already Even
    ...
    */
    ...
    {
        fnew->FFp(0) = fold->FFp(0);
        fnew->FFi(0) = fold->FFi(0);
        fnew->FFp(1) = fold->FFp(1);
        fnew->FFi(1) = fold->FFi(1);
        fnew->FFp(2) = fold->FFp(2);
        fnew->FFi(2) = fold->FFi(2);
        fnew->V(0) = fold->V(0);
        fnew->V(1) = fold->V(1);
        fnew->V(2) = fold->V(2);
    }
    ...
    */
    static void MakePureByRefine(MeshType &m)
    {
        ...
        for (FaceIterator fi = m.face.begin(); fi != m.face.end(); fi++)
            ...
            {
                int k = 0;
                if (face::IsBorder(*fi, 0))
                    k++;
                if (face::IsBorder(*fi, 1))
                    k++;
                if (face::IsBorder(*fi, 2))
                    k++;
                if (!fi->IsAnyF())
                {
                    // it's a triangle
        ...
        for (FaceIterator fi = m.face.begin(), fend = nfi; fi != fend; fi++)
            if (!fi->IsD() && !fi->IsV())
            {
                fi->SetV();
                ...
                if (!fi->IsAnyF())
                {
                    ...
                    if (k == 0)  // add a vertex in the center of the face, splitting it in 3
                    {
                        ...
                        if (fb->FFp(2) == fa)
                            fb->FFp(2) = fb;  // recover border status
                        if (fc->FFp(0) == fa)
                            fc->FFp(0) = fc;
                    ...
                    if (k == 1)
                    {  // make a border face faux, anf other two as well
                        fi->SetF(0);
                        fi->SetF(1);
                        fi->SetF(2);
                        nsplit++;
                    ...
                    if (k == 3)  // disconnected single triangle (all borders): use catmull-clark (tree vertices, split
                                 // it in 6
                    {
                        fi->ClearAllF();
                        fi->SetF(2);
                ...
                else
                {
                    ...
                    fc->ImportData(*fa);
                    CopyTopology(fc, fa);  // lazy: copy everything from the old vertex
                    fd->ImportData(*fb);
                    CopyTopology(fd, fb);  // lazy: copy everything from the old vertex

                    fa->V(ea2) = fc->V(ea0) = fb->V(eb2) = fd->V(eb0) = nv;
                    ...
                    if (bc)
                        fc->FFp(ea1) = fc;  // repristinate boundary status
                    if (bd)
                        fd->FFp(eb1) = fd;  // of new faces
        ...
        if (nsplit > 0)
        {
            ...
            for (FaceIterator fi = m.face.begin(), fend = nfi; fi != fend; fi++)
                if (!fi->IsD())
                {
                    ...
                    if (fa->FFp(0) == fa && fa->IsF(0))
                        ea2 = 0;
                    if (fa->FFp(1) == fa && fa->IsF(1))
                        ea2 = 1;
                    if (fa->FFp(2) == fa && fa->IsF(2))
                        ea2 = 2;

                    if (ea2 != -1)
                    {  // ea2 edge is naughty (border AND faux)
                        ...
                        if (fc->FFp(ea1) == fa)
                            fc->FFp(ea1) = fc;  // recover border status
    ...
    */
    static int MakePureByFlipStepByStep(MeshType &m, int maxdist = 10000, int restart = false)
    {
        ...
        if (step == 0)
        {
            ...
            if (!ta)
                return 0;  // success: no triangle left (done?)

            tb = MarkEdgeDistance(m, ta, maxdist);
            if (!tb)
                return 1;  // fail: no matching triagle found (increase maxdist?)
    ...
    */
    static bool MakePureByFlip(MeshType &m, int maxdist = 10000)
    {
        ...
        int res = -1;
        while (res == -1)
            res = MakePureByFlipStepByStep(m, maxdist);
        return res == 0;
    }
