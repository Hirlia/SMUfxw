...
class OverlapEstimation
{
  ...
  private:
    ...
    class VertexPointerSampler
    {
      public:
        ...
        }  // this function is the only we really need
  ...
  public:
    ...
    int Check(Parameters& param)
    {
        ...
        {
            assert(normBuckets);
            for (unsigned int i = 0; i < mMov->vert.size(); i++)
                queryVert.push_back(&(mMov->vert[i]));  // do a copy of pointers to vertexes
        ...
        for (i = 0, vi = queryVert.begin(); vi != queryVert.end(); vi++, i++)
        {
            ...
            if (mFix->bbox.IsIn(queryPnt))
                closestVertex = gridFix->GetClosest(PDistFunct, markerFunctorFix, queryPnt, consDist, dist, closestPnt);
            else
                closestVertex = NULL;  // out of bbox, we consider the point not in consensus...

            if (closestVertex != NULL && dist < consDist)
            {
                assert(closestVertex->P() ==
                       closestPnt);  // coord and vertex pointer returned by getClosest must be the same
                ...
                if (queryNrm.dot(closestVertex->N()) > param.consensusNormalsAngle)  // 15 degrees
                {
                    ...
                    if (param.paint)
                        (*vi)->Q() = 0.0f;  // store 0 as quality
                }
                else
                {
                    if (param.paint)
                        (*vi)->Q() = 1.0f;  // store 1 as quality
                }
            }
            else
            {
                if (param.paint)
                    (*vi)->Q() = 2.0f;  // store 2 as quality
            }
        }

        // Paint the mesh only if required and if consensus is the best ever found. Colors have been stores as numbers
        // into quality attribute
        if (param.paint)
        {
  ...
  private:
    ...
    void SampleVertUniform(MESH_TYPE& m, vector<typename MESH_TYPE::VertexPointer>& vert, int sampleNum)
    {
        ...
        for (unsigned int i = 0; i < sampler.sampleVec.size(); i++)
            vert.push_back(sampler.sampleVec[i]);
    }
    ...
    vector<vector<int> >* BucketVertexNormal(typename MESH_TYPE::VertContainer& vert, int bucketDim = 30)
    {
        ...
        if (NV.size() == 0)
            GenNormal<float>::Uniform(bucketDim, NV);

        vector<vector<int> >* BKT =
          new vector<vector<int> >(NV.size());  // NV size is greater then bucketDim, so don't change this!
