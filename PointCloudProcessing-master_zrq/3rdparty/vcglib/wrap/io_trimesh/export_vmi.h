...
class ExporterVMI
{
    ...
    struct SaveFaceOcf<MeshType, face::vector_ocf<typename MeshType::FaceType> >
    {
        ...
        SaveFaceOcf(FILE *f, const face::vector_ocf<FaceType> &face, bool only_header)
        {
            ...
            if (FaceType::HasWedgeColorOcf() && face.IsWedgeColorEnabled())
            {
                ...
                if (!only_header)
                    fwrite(&face.WCV[0], sizeof(typename face::vector_ocf<FaceType>::WedgeColorTypePack), face.size(),
                           f);
            }
            else
                WriteString(f, "NOT_HAS_FACE_WEDGECOLOR_OCF");
            ...
            if (FaceType::HasWedgeNormalOcf() && face.IsWedgeNormalEnabled())
            {
                ...
                if (!only_header)
                    fwrite(&face.WNV[0], sizeof(typename face::vector_ocf<FaceType>::WedgeNormalTypePack), face.size(),
                           f);
            }
            else
                WriteString(f, "NOT_HAS_FACE_WEDGENORMAL_OCF");
  ...
  public:
    static int Save(const SaveMeshType &m, const char *filename)
    {
        ...
        F() = fopen(filename, "wb");
        if (F() == NULL)
            return 1;  // 1 is the error code for cant'open, see the ErrorMsg function
        ...
        for (i = 0; i < nameF.size(); ++i)
            WriteString(F(), nameF[i].c_str());
        ...
        {
            float_value = m.bbox.min[i];
            WriteFloat(F(), float_value);
        }
        for (unsigned int i = 0; i < 2; ++i)
        {
            float_value = m.bbox.max[i];
            WriteFloat(F(), float_value);
        }
        ...
        /* save the per vertex attributes */
        {
            ...
            for (ai = m.vert_attr.begin(); ai != m.vert_attr.end(); ++ai)
                n_named_attr += !(*ai)._name.empty();

            WriteString(F(), "N_PER_VERTEX_ATTRIBUTES");
        ...
        ///* save the per mesh attributes */
        {
            ...
            for (ai = m.mesh_attr.begin(); ai != m.mesh_attr.end(); ++ai)
                n_named_attr += !(*ai)._name.empty();
            WriteString(F(), "N_PER_MESH_ATTRIBUTES");
            WriteInt(F(), n_named_attr);
            for (ai = m.mesh_attr.begin(); ai != m.mesh_attr.end(); ++ai)
                if (!(*ai)._name.empty())
                {
    ...
    static const char *ErrorMsg(int error)
    {
        ...
        if (error > 1 || error < 0)
            ...
            return off_error_msg[error].c_str();
    }
};  // end class
