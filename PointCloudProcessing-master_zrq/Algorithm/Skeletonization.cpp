...
void Skeletonization::runAutoWlopOneStep()
{
        ...
        iterate_time_in_one_stage > para->getDouble("Max Iterate Time"))
    {
        ...
        if (left_moving_num <= para->getDouble("Accept Branch Size") || current_radius > stop_radius)
        {
            finalProcess();
        }
...
double Skeletonization::wlopIterate()
{
    ...
    GlobalFun::computeEigenWithTheta(samples,
                                     para->getDouble("CGrid Radius") / sqrt(para->getDouble("H Gaussian Para")));
    time.end();
...
void Skeletonization::eigenThresholdIdentification()
{
    ...
    GlobalFun::computeAnnNeigbhors(samples->vert, samples->vert, sigma_KNN, false,
...
void Skeletonization::searchNewBranches()
{
    ...
    GlobalFun::computeAnnNeigbhors(samples->vert, samples->vert, branch_KNN, false,
                                   "void Skeletonization::searchNewBranches()");
...
Branch Skeletonization::searchOneBranchFromDirection(int begin_idx, Point3f head_direction)
{
    ...
    do
    {
        ...
        if (angle > para->getDouble("Branches Search Angle") || !next_v.is_fixed_sample || next_v.is_skel_branch ||
            next_v.is_skel_virtual)
        {
...
void Skeletonization::growVirtualTailUntilStop(Branch& branch)
{
    ...
    while (is_tail_growing)
    {
        ...
        else
        {
            ...
            if (angle0 >= 0 && angle1 >= 0 && angle0 < angle_threhold && angle1 < angle_threhold)
            {
...
void Skeletonization::labelFixOriginal()
{
    ...
    {
        case 1:
            GlobalFun::computeBallNeighbors(samples, original, para->getDouble("CGrid Radius") /
                                                                 sqrt(para->getDouble("H Gaussian Para")) * pca_para,
                                            original->bbox);
        ...
        case 2:
            ...
            GlobalFun::computeBallNeighbors(samples, original, para->getDouble("Local Density Radius") /
                                                                 sqrt(para->getDouble("H Gaussian Para")) * pca_para,
                                            original->bbox);
    ...
    if (branch.getTailAngle() < para->getDouble("Bad Virtual Angle"))
    {
        ...
        for (int i = 0; i < branches.size(); i++)
        {
            ...
            for (int j = 0; j < curve1.size(); j++)
            {
                ...
                if (dist_tail_t2 < MAX_Merge_Dist2 && dist_tail_t2 < min_dist && tail_direction * near_direction > 0)
                {
    ...
    if (branch.isTailVirtual())
    {
        ...
        else
        {
            ...
            if (!v.neighbors.empty())
            {
                ...
                Point3f real_tail_direction =
                  (curve0[curve0.size() - 2].P() - curve0[curve0.size() - 3].P()).Normalize();
