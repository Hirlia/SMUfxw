...
struct Hinge
{
    ...
    Hinge(int _v0, int _v1, int _v2, int _face, Point3f &_center)
        : v0(_v0), v1(_v1), v2(_v2), face(_face), center(_center), count(0)
    {
    ...
    bool seed(bool outside = true, int start = -1)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for (int k = i + 1; k < n; k++)
            {
                ...
                if (vv1.IsD() || vv1.IsB() || vv1.IsV())
                    continue;
                Point3f &p1 = mesh.vert[v1].P();
                if ((p1 - p0).Norm() < mindist * radius)
                    continue;
                ...
                for (int j = k + 1; j < n; j++)
                {
                    ...
                    if (vv2.IsD() || vv2.IsB() || vv2.IsV())
                        continue;
                    Point3f &p2 = mesh.vert[v2].P();
                    if ((p2 - p0).Norm() < mindist * radius)
                        continue;
                    if ((p2 - p1).Norm() < mindist * radius)
                        continue;
    ...
    bool pivot(Hinge &Hinge, int &candidate, Point3f &end_pivot, std::vector<int> &targets)
    {
        ...
        */
        ...
        for (int i = 0; i < targets.size(); i++)
        {
            ...
            if (id == Hinge.v0 || id == Hinge.v1 || id == Hinge.v2)
                continue;
  ...
  private:
    ...
    bool checkHinge(int v0, int v1)
    {
        ...
        for (; i < mesh.face.size(); i++)
        {
            ...
            for (int k = 0; k < 3; k++)
            {
                if (v1 == (int)f.V(k) && v0 == (int)f.V((k + 1) % 3))
                    ++tot;
                else if (v0 == (int)f.V(k) && v1 == (int)f.V((k + 1) % 3))
                {  // orientation non constistent
                    return false;
                }
            }
            if (tot >= 2)
