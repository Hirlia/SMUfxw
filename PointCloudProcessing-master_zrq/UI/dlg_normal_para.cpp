...
void NormalParaDlg::initConnects()
{
    ...
    connect(ui->normal_pvcmn,SIGNAL(clicked()), this, SLOT(applyPCVMNNormal()));
    ...
    connect(ui->feature_threshold,SIGNAL(valueChanged(double)), this, SLOT(getFeatureThreshold(double)));
    connect(ui->index,SIGNAL(valueChanged(int)), this, SLOT(getIndex(int)));
    connect(ui->sigma_n,SIGNAL(valueChanged(double)), this, SLOT(getsigman(double)));
    connect(ui->eigenvector,SIGNAL(clicked()), this, SLOT(calculate_eigenvector()));
}
...
bool NormalParaDlg::initWidgets()
{
    ...
    ui->sigma_n->setValue(m_paras->norSmooth.getDouble("sigma n"));
    ui->index->setValue(m_paras->norSmooth.getInt("Index"));
    ui->smooth_normal_interate_num->setValue(m_paras->norSmooth.getInt("Number Of Iterate"));
    Qt::CheckState state =
      m_paras->norSmooth.getBool("Run Anistropic PCA") ? (Qt::CheckState::Checked) : (Qt::CheckState::Unchecked);
...
void NormalParaDlg::getsigman(double _val)
{
    m_paras->norSmooth.setValue("sigma n",DoubleValue(_val));
}
...
void NormalParaDlg::getFeatureThreshold(double _val)
{
    m_paras->norSmooth.setValue("Feature threshold",DoubleValue(_val));
}

void NormalParaDlg::getIndex(int _val)
{
    m_paras->norSmooth.setValue("Index",IntValue(_val));
}
...
void NormalParaDlg::applyPCANormal()
{
    ...
    else
    {
        ...
        CMesh* samples = area->dataMgr.getCurrentSamples();
        CMesh* samples1 = area->dataMgr.getCurrentNoise();
        vcg::NormalExtrapolation<vector<CVertex> >::ExtrapolateNormals(samples->vert.begin(), samples->vert.end(), knn,
                                                                       -1);
        vcg::NormalExtrapolation<vector<CVertex> >::ExtrapolateNormals(samples1->vert.begin(), samples1->vert.end(), knn,
                                                                       -1);
    }
    area->dataMgr.recomputeQuad();
    area->updateGL();
...
void NormalParaDlg::applyPCVMNNormal()
{
    int knn = global_paraMgr.norSmooth.getInt("PCA KNN");
    CMesh* samples = area->dataMgr.getCurrentSamples();
    GlobalFun::computeAnnNeigbhors(samples->vert, samples->vert, 2*knn, false, "pcvmn big neighbors");
    PCVmn::compute_normals(samples->vert.begin(), samples->vert.end(),knn);
    area->dataMgr.recomputeQuad();
    area->updateGL();
}

void NormalParaDlg::calculate_eigenvector()
{
    int knn = global_paraMgr.norSmooth.getInt("PCA KNN");
    CMesh* samples = area->dataMgr.getCurrentSamples();
    vcg::NormalExtrapolation<vector<CVertex> >::calculate_normsigma(&area->dataMgr.samples,samples->vert.begin(), samples->vert.end(), knn);
    //area->dataMgr.recomputeQuad();
    area->updateGL();
}
