...
class TrivialSampler
{
  ...
  public:
    ...
    void AddTextureSample(const FaceType &, const CoordType &, const Point2i &, float)
    {
        ...
        // if edgeDist is > 0 then the corrisponding point is affecting face color even if outside the face area (in
        // texture space)
    }
...
#define FAK_LEN 1024
    ...
    static int PoissonRatioUniforms(double L)
    {
        ...
        This approach is STABLE even for large L (e.g. it does not suffer from the overflow limit of the classical Knuth
        implementation)
    ...
    /**
      This function compute montecarlo distribution with an approximate number of samples exploiting the poisson
      ...
      To approximate the Binomial we use a Poisson distribution with parameter \lambda = np can be used as an
      approximation to B(n,p) (it works if n is sufficiently large and p is sufficiently small).
    ...
    static void Montecarlo(MetroMesh &m, VertexSampler &ps, int sampleNum)
    {
        ...
        for (i = 0; i < sampleNum; ++i)
        {
            ...
            // lower_bound returns the furthermost iterator i in [first, last) such that, for every iterator j in
            // [first, i), *j < value.
            // E.g. An iterator pointing to the first element "not less than" val, or end() if every element is less
            // than val.
            typename std::vector<IntervalType>::iterator it =
              lower_bound(intervals.begin(), intervals.end(), std::make_pair(val, FacePointer(0)));
    ...
    /// So the a face with a zero quality on all its vertices is never sampled and a face with average quality 2 get
    /// twice the samples of a face with the same area but with an average quality of 1;
    ...
    static int SingleFaceSubdivision(int sampleNum, const CoordType &v0, const CoordType &v1, const CoordType &v2,
                                     VertexSampler &ps, FacePointer fp, bool randSample)
    {
        ...
        if (sampleNum == 1)
        {
            ...
            if (randSample)
            {
                ...
                SamplePoint = ((v0 + v1 + v2) * (1.0f / 3.0f));
    ...
    /// Compute a sampling of the surface where the points are regularly scattered over the face surface using a
    /// recursive longest-edge subdivision rule.
    ...
    static int SingleFaceSubdivisionOld(int sampleNum, const CoordType &v0, const CoordType &v1, const CoordType &v2,
                                        VertexSampler &ps, FacePointer fp, bool randSample)
    {
        // recursive face subdivision.
        if (sampleNum == 1)
    ...
    static void FaceSubdivisionOld(MetroMesh &m, VertexSampler &ps, int sampleNum, bool randSample)
    {
        ...
        for (fi = faceVec.begin(); fi != faceVec.end(); fi++)
        {
            ...
            faceSampleNum = (int)floatSampleNum;
            if (faceSampleNum > 0)
                faceSampleNum = SingleFaceSubdivision(faceSampleNum, (**fi).V(0)->cP(), (**fi).V(1)->cP(),
                                                      (**fi).V(2)->cP(), ps, *fi, randSample);
            floatSampleNum -= (double)faceSampleNum;
        }
    }
    ...
    static int SingleFaceSimilarDual(FacePointer fp, VertexSampler &ps, int n_samples_per_edge, bool randomFlag)
    {
        ...
        for (i = 0; i < n_samples_per_edge - 1; i++)
            ...
            {
                // AddSample( v0 + (V1*(double)i + V2*(double)j) );
                CoordType V0((i + 0) * segmentLen, (j + 0) * segmentLen,
                             1.0 - ((i + 0) * segmentLen + (j + 0) * segmentLen));
                CoordType V1((i + 1) * segmentLen, (j + 0) * segmentLen,
                             1.0 - ((i + 1) * segmentLen + (j + 0) * segmentLen));
                CoordType V2((i + 0) * segmentLen, (j + 1) * segmentLen,
                             1.0 - ((i + 0) * segmentLen + (j + 1) * segmentLen));
                n_samples++;
                if (randomFlag)
                {
                    CoordType rb = RandomBaricentric();
                    ...
                    ps.AddFace(*fp, (V0 + V1 + V2) / 3.0);

                if (j < n_samples_per_edge - i - 2)
                {
                    CoordType V3((i + 1) * segmentLen, (j + 1) * segmentLen,
                                 1.0 - ((i + 1) * segmentLen + (j + 1) * segmentLen));
                    n_samples++;
                    if (randomFlag)
                    {
                        CoordType rb = RandomBaricentric();
    ...
    // Similar sampling
    // Each triangle is subdivided into similar triangles following a generalization of the classical 1-to-4 splitting
    // rule of triangles.
    // According to the level of subdivision <k> you get 1, 4 , 9, 16 , <k^2> triangles.
    // Depending on the kind of the sampling strategies we can have two different approach to choosing the sample
    // points.
    // 1) you have already sampled both edges and vertices
    // 2) you are not going to take samples on edges and vertices.
    //
    // In the first case you have to consider only internal vertices of the subdivided triangles (to avoid multiple
    // sampling of edges and vertices).
    // Therefore the number of internal points is ((k-3)*(k-2))/2. where k is the number of points on an edge (vertex
    // included)
    ...
    //             2
    //
    // In the second case if you are not interested to skip the sampling on edges and vertices you have to consider as
    // sample number the number of triangles.
    ...
    static void FaceSimilar(MetroMesh &m, VertexSampler &ps, int sampleNum, bool dualFlag, bool randomFlag)
    {
        ...
        for (fi = m.face.begin(); fi != m.face.end(); fi++)
        {
            ...
            if (n_samples > 0)
            {
                ...
                {
                    n_samples_per_edge =
                      (int)((sqrt(1.0 + 8.0 * (double)n_samples) + 5.0) / 2.0);  // original for non dual case
    ...
    // This function does rasterization with a safety buffer area, thus accounting some points actually outside triangle
    // area
    // The safety area samples are generated according to face flag BORDER which should be true for texture space border
    // edges
        ...
        for (int x = bbox.min[0] - 1; x <= bbox.max[0] + 1; ++x)
        {
            ...
            for (int y = bbox.min[1] - 1; y <= bbox.max[1] + 1; ++y)
            {
                if ((n[0] >= 0 && n[1] >= 0 && n[2] >= 0) || (n[0] <= 0 && n[1] <= 0 && n[2] <= 0))
                {
                    ...
                    baryCoord[0] =
                      double(-y * v1[0] + v2[0] * y + v1[1] * x - v2[0] * v1[1] + v1[0] * v2[1] - x * v2[1]) / de;
                    baryCoord[1] =
                      -double(x * v0[1] - x * v2[1] - v0[0] * y + v0[0] * v2[1] - v2[0] * v0[1] + v2[0] * y) / de;
                ...
                else
                {
                    ...
                    // find the closest point (on some edge) that lies on the 2x2 squared neighborhood of the considered
                    // point
                    for (int i = 0; i < 3; ++i)
                    {
                        if (edgeMask & (1 << i))
                        {
                            ...
                            if ((!flipped && n[i] < 0 || flipped && n[i] > 0) &&
                                ...
                                close.X() > px.X() - 1 && close.X() < px.X() + 1 && close.Y() > px.Y() - 1 &&
                                close.Y() < px.Y() + 1)
                            {
                                minDst = dst;
                                closePoint = close;
                    ...
                    if (closeEdge >= 0)
                    {
                        ...
                        if (correctSafePointsBaryCoords)
                        {
                            ...
                            baryCoord[closeEdge] =
                              (closePoint - borderEdges[closeEdge].P(1)).Norm() / edgeLength[closeEdge];
                        ...
                        else
                        {
                            ...
                            baryCoord[0] =
                              double(-y * v1[0] + v2[0] * y + v1[1] * x - v2[0] * v1[1] + v1[0] * v2[1] - x * v2[1]) /
                              de;
                            baryCoord[1] =
                              -double(x * v0[1] - x * v2[1] - v0[0] * y + v0[0] * v2[1] - v2[0] * v0[1] + v2[0] * y) /
                              de;
                            baryCoord[2] = 1 - baryCoord[0] - baryCoord[1];
                        }
    ...
    struct PoissonDiskParam
    {
        ...
        bool invertQuality;
        bool preGenFlag;  // when generating a poisson distribution, you can initialize the set pof omputed points with
                          // ALL the vertices of another mesh. Usefull for building progressive refinements.
    ...
    static ScalarType ComputePoissonDiskRadius(MetroMesh &origMesh, int sampleNum)
    {
        ...
        if (meshArea == 0)
        {
            meshArea = (origMesh.bbox.DimX() * origMesh.bbox.DimY() + origMesh.bbox.DimX() * origMesh.bbox.DimZ() +
                                   ...
                                   ScalarType diskRadius, const struct PoissonDiskParam pp = PoissonDiskParam())
    {
        ...
        // if we are doing variable density sampling we have to prepare the random samples quality with the correct
        // expected radii.
                            ...
                            const struct PoissonDiskParam pp = PoissonDiskParam())
    {
        ...
        do
        {
            ...
            {  // initialize spatial hash with the remaining points
                montecarloSHT.InitEmpty(origMesh.bbox, gridsize);
                // create active cell list
                for (typename MontecarloSHT::HashIterator hi = montecarloSHTVec[level - 1].hash_table.begin();
                     hi != montecarloSHTVec[level - 1].hash_table.end(); hi++)
            ...
            std::random_shuffle(montecarloSHT.AllocatedCells.begin(), montecarloSHT.AllocatedCells.end(), p_myrandom);
#ifdef QT_VERSION
            qDebug("PDS: Init of Hashing grid %i cells and %i samples (%i msec)", montecarloSHT.AllocatedCells.size(),
                   montecarloSHT.hash_table.size(), tt.restart());
#endif
            ...
            for (int i = 0; i < montecarloSHT.AllocatedCells.size(); i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    ...
                    typename MontecarloSHT::HashIterator hi =
                      montecarloSHT.hash_table.find(montecarloSHT.AllocatedCells[i]);

                    if (hi == montecarloSHT.hash_table.end())
                    {
                        break;
                    }
...
//
#ifdef QT_VERSION
            qDebug("PDS: Pruning %i added %i and removed %i samples (%i msec)", level, addedCnt, removedCnt,
                   tt.restart());
#endif
            level++;
        } while (level < 5);
    ...
    static void SubdivideAndSample(MetroMesh &m, std::vector<Point3f> &pvec, const Box3<ScalarType> bb, RRParam &rrp,
                                   float curDiag)
    {
        ...
        if (dist < dist_upper_bound)
        {
            if (curDiag / 3 < rrp.minDiag)  // store points only for the last level of recursion (?)
            {
                ...
                {
                    if (dist > rrp.offset)  // points below the offset threshold cannot be displaced at the right offset
                                            // distance, we can only make points nearer.
                    {
                    ...
                    for (int k = 0; k < 2; k++)
                        SubdivideAndSample(
                          ...
                          Box3f(Point3f(bb.min[0] + i * hs[0], bb.min[1] + j * hs[1], bb.min[2] + k * hs[2]),
                                Point3f(startPt[0] + i * hs[0], startPt[1] + j * hs[1], startPt[2] + k * hs[2])),
                          rrp, curDiag);
        }
    }
};  // end class
