...
class glLabel
{
  ...
  public:
    ...
    static void render2D(QPainter *painter, const LabelPosition pos, int linePos, const QString &text,
                         const Mode &m = Mode())
    {
        Mode lm = m;
        ...
        {
            case TOP_LEFT:
                render2D(painter, vcg::Point2f(delta, view[3] - 3 * delta - delta * 3 * linePos), text, lm);
                break;
            case TOP_RIGHT:
                render2D(painter, vcg::Point2f(view[2] - delta, view[3] - 3 * delta - delta * 3 * linePos), text, lm);
                break;
            case BOTTOM_LEFT:
                render2D(painter, vcg::Point2f(delta, 3 * delta + delta * 3 * linePos), text, lm);
                break;
            case BOTTOM_RIGHT:
                render2D(painter, vcg::Point2f(view[2] - delta, 3 * delta + delta * 3 * linePos), text, lm);
                break;
        }
    }
