...
bool DataMgr::isNoiseEmpty()
{
    return noise.vert.empty();
}
...
void DataMgr::loadPlyToSample(QString fileName)
{
    ...
    //tri::UpdateTopology<CMesh>::VertexFace(samples);
    CMesh::VertexIterator vi;
    int idx = 0;
...
void DataMgr::loadPlyToNoise(QString fileName)
{
    clearCMesh(noise);
    curr_file_name = fileName;
    ...
    int err = tri::io::Importer<CMesh>::Open(noise, curr_file_name.toAscii().data(), mask);
    if (err)
    {
        cout << "Failed reading mesh: " << err << "\n";
        return;
    ...
    CMesh::VertexIterator vi;
    int idx = 0;
    for (vi = noise.vert.begin(); vi != noise.vert.end(); ++vi)
    {
        vi->bIsOriginal = false;
        vi->m_index = idx++;
        noise.bbox.Add(vi->P());
    }
    noise.vn = noise.vert.size();
}
...
double DataMgr::getInitRadiuse()
{
    ...
    if (!isOriginalEmpty())
    {
        ...
        if (abs(box.min.X() - box.max.X()) < 1e-5 || abs(box.min.Y() - box.max.Y()) < 1e-5 ||
