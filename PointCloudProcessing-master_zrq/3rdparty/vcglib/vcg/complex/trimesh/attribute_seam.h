...
class AttributeSeam
{
  public:
    ...
    struct ASExtract
    {
        ...
        void operator()(const src_trimesh_t &sm, const typename src_trimesh_t::FaceType &f, int k,
                        const dst_trimesh_t &dm, typename dst_trimesh_t::VertexType &v) const
        {
            (void)sm;
            (void)dm;
            ...
            if ((m & AttributeSeam::NORMAL_PER_VERTEX) != 0)
                v.N() = u.cN();
            if ((m & AttributeSeam::NORMAL_PER_WEDGE) != 0)
                v.N() = f.cWN(k);
            if ((m & AttributeSeam::NORMAL_PER_FACE) != 0)
                v.N() = f.cN();

            if ((m & AttributeSeam::COLOR_PER_VERTEX) != 0)
                v.C() = u.cC();
            if ((m & AttributeSeam::COLOR_PER_WEDGE) != 0)
                v.C() = f.cWC(k);
            if ((m & AttributeSeam::COLOR_PER_FACE) != 0)
                v.C() = f.cC();

            if ((m & AttributeSeam::TEXCOORD_PER_VERTEX) != 0)
                v.T() = u.cT();
            if ((m & AttributeSeam::TEXCOORD_PER_WEDGE) != 0)
                v.T() = f.cWT(k);
        }
    };
    ...
    struct ASCompare
    {
        ...
        bool operator()(const dst_trimesh_t &sm, const typename dst_trimesh_t::VertexType &u,
                        const typename dst_trimesh_t::VertexType &v) const
        {
            (void)sm;
            ...
            */
            ...
            if ((m & (AttributeSeam::NORMAL_PER_VERTEX | AttributeSeam::NORMAL_PER_WEDGE |
                      AttributeSeam::NORMAL_PER_FACE)) != 0)
            {
                if (u.cN() != v.cN())
                    return false;
            }

            if ((m & (AttributeSeam::COLOR_PER_VERTEX | AttributeSeam::COLOR_PER_WEDGE |
                      AttributeSeam::COLOR_PER_FACE)) != 0)
            {
                if (u.cC() != v.cC())
                    return false;
            }
    ...
    static inline bool SplitVertex(src_trimesh_t &src, extract_wedge_attribs_t v_extract,
                                   compare_vertex_attribs_t &v_compare)
    {
        ...
        typedef typename src_mesh_allocator_t::template PointerUpdater<typename src_trimesh_t::VertexPointer>
          src_pointer_updater_t;
        ...
        for (src_face_i it = src.face.begin(); it != src.face.end(); ++it)
        {
            ...
            for (int k = 0; k < 3; ++k)
            {
                ...
                else
                {
                    ...
                    do
                    {
                        if (v_compare(src, src.vert[vidx], *vtx))
                            break;
                        vidx = vloc[vidx];
                    } while (vidx >= 0);
    ...
    template <typename src_trimesh_t, typename dst_trimesh_t, typename extract_wedge_attribs_t,
              typename compare_vertex_attribs_t, typename copy_vertex_t>
    static inline bool SplitVertex(const src_trimesh_t &src, dst_trimesh_t &dst, extract_wedge_attribs_t &v_extract,
                                   compare_vertex_attribs_t &v_compare, copy_vertex_t &v_copy)
    {
        typedef typename src_trimesh_t::VertexType src_vertex_t;
        typedef typename src_trimesh_t::FaceType src_face_t;
        ...
        typedef typename dst_mesh_allocator_t::template PointerUpdater<typename dst_trimesh_t::VertexPointer>
          dst_pointer_updater_t;
        ...
        for (src_face_ci it = src.face.begin(); it != src.face.end(); ++it)
        {
            ...
            for (int k = 0; k < 3; ++k)
            {
                ...
                else
                {
                    ...
                    do
                    {
                        if (v_compare(dst, dst.vert[vidx], *vtx))
                            break;
                        vidx = vloc[vidx];
                    } while (vidx >= 0);
