...
class AnistropicPca
{
  ...
  public:
    ...
    static void SmoothNormalsUsingNeighbors(const VertexIterator &begin, const VertexIterator &end,
                                            const unsigned int k, bool usedistance, CallBackPos *callback = NULL)
    {
        ...
        for (VertexIterator iter = begin; iter != end; iter++)
        {
            ...
            octree_for_neighbors.GetKClosest(vpdf, dom, k, iter->P(), max_distance, nearest_vertices, distances,
                                             nearest_points);
