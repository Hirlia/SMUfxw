  ...
  public:
    ...
    */
    ...
    static ScalarType EdgeLenghtVariationIfVertexRotated(const FaceType &f, int w0)
    {
        ...
        ScalarType before = 0,  // sum of quad edges (originating from v)
        ...
        do
        {
            ...
            t = pf->FFp(pi);
            if (pf == t)
                return std::numeric_limits<ScalarType>::max();  // it's a mesh border! flee!
            pi = pf->cFFi(pi);
    ...
    static ScalarType QuadQualityVariationIfVertexRotated(const FaceType &f, int w0)
    {
        ...
        ScalarType before = 0,  // sum of quad quality around v
...
// rotate vertex IFF overall Quality increase
    ...
    static bool RotateVertex(FaceType &f, int w0, MeshType &m, Pos *affected = NULL)
    {
        ...
        do
        {
            ...
            if (mustFlip)
            {
                if (!CheckFlipDiag(*lastF))
                    return false;  // cannot flip??
                FlipDiag(*lastF);
            }
            MarkFaceF(pf);
        ...
        do
        {
            ...
            {
                pf->ClearF(j);
                IncreaseValency(pf->V1(j));
            }
            else
            {
                pf->SetF(j);
                DecreaseValencySimple(pf->V1(j), 1);
            }
    ...
    static void FlipEdge(FaceType &f, int k, MeshType &m)
    {
        ...
        for (int k = 0; k < 3; k++)
        {
            ...
            if (fa->FFp(k)->IsF(fa->FFi(k)))
                fa->SetF(k);
            if (fb->FFp(k)->IsF(fb->FFi(k)))
                fb->SetF(k);
        }
    }
    ...
    {
        if (affected)
            affected->mode = Pos::NOTHING;  // singlets leave nothing to update behind

        if (f.V(wedge)->IsB())
            return;  // hack: lets detect
    ...
    static bool IsDoubletFF(const FaceType &f, int wedge)
    {
        ...
        {
            if (!pf->IsAnyF())
                return false;  // there's a triangle!
            if (!pf->IsF(pi))
                res++;
            const FaceType *t = pf;
            t = pf->FFp(pi);
    ...
    {
        if (GetValency(f.V(wedge)) == 2)
        {
            RemoveDoublet(f, wedge, m, affected);
            return true;
        }
    ...
    static bool CollapseEdge(FaceType &f, int w0, MeshType &m, Pos *affected = NULL)
    {
        ...
        if (IsDoubletOrSinglet(f, w0))
            return false;  //{ RemoveDoubletOrSinglet(f,w0,m, affected); return true;}
        if (IsDoubletOrSinglet(f, (w0 + 1) % 3))
            return false;  //{ RemoveDoubletOrSinglet(f,(w0+1)%3,m, affected); return true;}
        ...
        if (EdgeLenghtVariationIfVertexRotated(*f0, w0) < EdgeLenghtVariationIfVertexRotated(*f1, w1))
            return CollapseEdgeDirect(*f0, w0, m);
        else
            return CollapseEdgeDirect(*f1, w1, m);
    }
    ...
    */
    ...
    static bool CollapseDiag(FaceType &f, ScalarType interpol, MeshType &m, Pos *affected = NULL)
    {
        ...
        if (IsDoubletOrSinglet(f, (fauxa + 2) % 3))
            return false;
        if (IsDoubletOrSinglet(*(f.FFp(fauxa)), (f.FFi(fauxa) + 2) % 3))
            return false;
        ...
        {
            // pf->V(pi) = va;
            if (((pf->V2(pi) == va) || (pf->V1(pi) == va)) && (pf != fa) && (pf != fb))
                return false;
            pi = (pi + 2) % 3;
    ...
    */
    ...
    {
        CoordType a = f->V0(edge)->P(), b = f->FFp(edge)->V2(f->FFi(edge))->P(), c = f->V1(edge)->P(),
                  d = f->V2(edge)->P();
    ...
    */
    static int TestEdgeRotation(const FaceType &f, int w0, ScalarType *gain = NULL)
    {
        ...
        if (q0 <= q1 && q0 <= q2)
            return 0;  // there's no rotation shortening this edge
        ...
        {
            if (gain)
                *gain = sqrt(q1) - sqrt(q0);
            // test: two diagonals should become shorter (the other two reamin the same)
            if ((v0 - v2).SquaredNorm() < (v4 - v2).SquaredNorm() || (v3 - v5).SquaredNorm() < (v1 - v5).SquaredNorm())
            {
                // stop++;
                return 0;
            }
        ...
        {
            if (gain)
                *gain = sqrt(q2) - sqrt(q0);
            // diagonal test, as above:
            if ((v0 - v4).SquaredNorm() < (v2 - v4).SquaredNorm() || (v3 - v1).SquaredNorm() < (v5 - v1).SquaredNorm())
            {
                // stop++;
                return 0;
            }
  ...
  public:
    ...
    static void QuadTriangulate(std::vector<VertexPointer> &q)
    {
        ...
        bool qualityImprove = std::min(QualityRadii(P0, P1, P2), QualityRadii(P0, P2, P3)) <
                              std::min(QualityRadii(P1, P2, P3), QualityRadii(P1, P3, P0));
        ...
        if (q[0] < q[2])
            res = diagSet.insert(std::make_pair(q[0], q[2]));
        else
            res = diagSet.insert(std::make_pair(q[2], q[0]));
