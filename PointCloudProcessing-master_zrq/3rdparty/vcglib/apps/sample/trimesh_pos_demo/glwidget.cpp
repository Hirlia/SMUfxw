...
void GLWidget::paintGL()
{
    ...
    if (doPickPos)
    {
        ...
        int yes = vcg::Pick<MyStraightMesh::FaceContainer>(pic_x, ScreenH - pic_y + 1, mesh.face, res,
                                                           vcg::glTriangle3<MyStraightMesh::FaceType>, 1, 1);
        if (yes)
        {
            fp = res[0];
            pos.Set(fp, 0, fp->V(0));
    ...
    else if (doPickVfIte)
    {
        ...
        int yes = vcg::Pick<MyStraightMesh::VertContainer>(pic_x, ScreenH - pic_y + 1, mesh.vert, res,
                                                           drawVertex<MyStraightMesh::VertexType>, 3, 3);
        if (yes)
        {
            vp = res[0];
