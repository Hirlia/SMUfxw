...
and viceversa. In a trimesh, the generic polygons with n sides are codified represented by tagging the internal edge of
the face
with the SetF.
*/
...
struct PolygonSupport
{
    ...
    static void ImportFromPolyMesh(TriMeshType &tm, PolyMeshType &pm)
    {
        ...
        for (tvi = tm.vert.begin(), vi = pm.vert.begin(); tvi != tm.vert.end(); ++tvi, ++vi, ++cnt)
            if (!(*vi).IsD())
                ...
                vcg::tri::Allocator<TriMeshType>::DeleteVertex(tm, (*tvi));
        ...
        for (fi = pm.face.begin(); fi != pm.face.end(); ++fi)
            if (!((*fi).IsD()))
            {
                ...
                for (int i = 0; tfi != tm.face.end(); ++tfi)
                {
                    ...
                    // set the F flags
                    if ((faces[i] + 1) % points.size() != faces[i + 1])
                        (*tfi).SetF(0);
                    if ((faces[i + 1] + 1) % points.size() != faces[i + 2])
                        (*tfi).SetF(1);
                    if ((faces[i + 2] + 1) % points.size() != faces[i])
                        (*tfi).SetF(2);
                    i += 3;
                }
            }
    ...
    **/
    static void ImportFromTriMesh(PolyMeshType &pm, TriMeshType &tm)
    {
        ...
        for (tvi = tm.vert.begin(); tvi != tm.vert.end(); ++tvi, ++vi, ++cnt)
            if (!(*tvi).IsD())
                ...
                vcg::tri::Allocator<PolyMeshType>::DeleteVertex(pm, (*vi));

        // convert the faces
        typename TriMeshType::FaceIterator tfi;
        vcg::face::JumpingPos<typename TriMeshType::FaceType> p;

        for (tfi = tm.face.begin(); tfi != tm.face.end(); ++tfi)
            if (!(*tfi).IsD() && !(*tfi).IsV())
            {
    ...
    static void ExtractPolygon(typename TriMeshType::FacePointer tfi,
