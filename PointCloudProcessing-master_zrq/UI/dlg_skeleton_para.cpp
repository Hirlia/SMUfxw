...
void SkeletonParaDlg::initConnects()
{
    ...
    connect(ui->wlop_clean_near_branch_dist, SIGNAL(valueChanged(double)), this,
            SLOT(getCleanNearbyBranchesDist(double)));
...
bool SkeletonParaDlg::initWidgets()
{
    ...
    state =
      m_paras->skeleton.getBool("Need Segment Right Away") ? (Qt::CheckState::Checked) : (Qt::CheckState::Unchecked);
...
{
    m_paras->setGlobalParameter("H Gaussian Para", DoubleValue(_val));
    update();
    area->update();
}
...
void SkeletonParaDlg::guessParaMeter()
{
    ...
    double influence_radius =
      m_paras->skeleton.getDouble("CGrid Radius") / sqrt(m_paras->skeleton.getDouble("H Gaussian Para"));
