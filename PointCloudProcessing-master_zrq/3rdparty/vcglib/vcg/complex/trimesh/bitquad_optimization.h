  ...
  public:
    ...
    static void MarkVertex(FaceType *f, int wedge, MeshType &m)
    {
        ...
        for (FaceIterator fi = m.face.begin(); fi != m.face.end(); fi++)
            if (!fi->IsD())
            {
                if (fi->V0(0) == v || fi->V1(0) == v || fi->V2(0) == v)
                    fi->Q() = 0;
                // else fi->Q() = 1;
            }
    }
        ...
        for (FaceIterator fi = m.face.begin(); fi != m.face.end(); fi++)
            if (!fi->IsD())
                for (int k = 0; k < 3; k++)
                {
                    ...
                    if (f->FFp(k) >= f)
                        continue;  // skip borders (==), and do it one per edge
                    ...
                    if (!f->IsF(k))
                        score *= edgeMult;  // edges are supposed to be smaller!
                    ...
                    if (f->IsF(k))
                    {  // for diag faces, test counterdiag too
        ...
        if (fa)
        {
            if (perform)
            {
                if (fa->IsF(w))
                {
                    if (counterDiag)
                    {
                        if (BQ::CollapseCounterDiag(*fa, BQ::PosOnDiag(*fa, true), m, affected))
                            return 2;
                    }
                    else
                    {
                        if (BQ::CollapseDiag(*fa, BQ::PosOnDiag(*fa, false), m, affected))
                            return 2;
                    }
                }
                else
                {
                    if (BQ::CollapseEdge(*fa, w, m, affected))
                        return 1;
                }
            }
            else
    ...
    static bool IdentifyAndCollapseSmallestDiag(MeshType &m)
    {
        ...
        if (BQ::TestAndRemoveDoublet(*fa, 0, m))
        {
            return true;
        }
        if (BQ::TestAndRemoveDoublet(*fa, 1, m))
        {
            return true;
        }
        if (BQ::TestAndRemoveDoublet(*fa, 2, m))
        {
            return true;
        }
        int k = BQ::FauxIndex(fa);
        if (BQ::TestAndRemoveDoublet(*fa->FFp(k), (fa->FFi(k) + 2) % 3, m))
            return true;
            ...
            if (!fi->IsD())
            {
                ...
                for (int k = 0; k < 3; k++)
                {
                    if (BQ::IsDoublet(*fi, k))
                    {
                        ...
                        if (fi->IsD())
                            break;  // break wedge circle, if face disappeard
                        if (p)
                            return res;
                    }
                }
            ...
            if (!fi->IsD())
            {
                for (int k = 0; k < 3; k++)
                {
                    ...
                    if (BQ::TestVertexRotation(*fi, k))
                    {
                        ...
                        {
                            if (BQ::RotateVertex(*fi, k, m, affected))
                                res++;  // fi->Q()=0;
                            if (affected)
                                return res;  // uncomment for only one rotation
                        }
                    }
                }
            }
            ...
            if (!fi->IsD())
            {
                ...
                for (int k = 0; k < 3; k++)
                {
                    if (fi->IsF(k))
                        continue;
                    if (fi->FFp(k) <= &*fi)
                        continue;  // only once per real (non faux) edge, and only for non border ones
                    int best = BQ::TestEdgeRotation(*fi, k);
                    if (perform)
                    {
                        if (best == +1)
                            if (BQ::template RotateEdge<true>(*fi, k, m, p))
                                count++;
                        if (best == -1)
                            if (BQ::template RotateEdge<false>(*fi, k, m, p))
                                count++;
                        if (p)
                            if (count > 0)
                                return count;
