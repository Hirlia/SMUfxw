...
{
  public:
    GLColor(const double& _r = 0, const double& _g = 0, const double& _b = 0, const double& _a = 1.0)
        : r(_r), g(_g), b(_b), a(_a)
    {
    }
...
class GLDrawer
{
  public:
    ...
    ~GLDrawer(void);
    typedef enum { DOT, QUADE, CIRCLE, NORMAL, SPHERE ,Face,EV,NEV} DrawType;

    void setViewPoint(const Point3f& view)
    {
        view_point = view;
    }
    ...
    void setRGBNormals(vector<Point3f>& normals)
    {
        RGB_normals = normals;
    }
  ...
  private:
    ...
    void drawEV(const CVertex& v);
    void drawNEV(const CVertex& v);
