...
{
    strcpy(dir, path);//C 库函数 char *strcpy(char *dest, const char *src) 把 src 所指向的字符串复制到 dest。
    char *p;
...
{
    if (pb_access(fname, 4) == -1)
        return false;
    if (pb_access(cname, 4) == -1)
        return false;
...
static bool CheckBBoxCache(const char *fname, Box3<ScalarType> &box, const char *fname2 = 0)
{
    ...
    if (CheckCacheTime(fname, d) && (fname2 == 0 || CheckCacheTime(fname2, d)))
    {
        ...
        if (fread(&readBB, sizeof(Box3f), 1, fp) != 1)
        {
            fclose(fp);
            return false;
        }
...
bool GetCacheName(const char *fname, const char *ext_name, char *cname)
{
    ...
    // Estrazione dati
    if (!GetDirFromPath(fname, cname, n))
        return false;
...
bool ScanBBox(const char *fname, Box3<ScalarType> &box, bool use_cache = true)
{
    ...
    static const PropDescriptor pv[3] = {
        { "vertex", "x", T_FLOAT, T_DOUBLE, offsetof(PlyPoint3d, x), 0, 0, 0, 0, 0,
          0 },  // TO GET RID OF COMPILER WARNING I ADDED 0 TO INITIALIZE format (MV)
    ...
    if (pf.AddToRead(pv[0]) == -1)
    {
        fprintf(stderr, "Warning: Read error\n");
        return false;
    }
    if (pf.AddToRead(pv[1]) == -1)
    {
        fprintf(stderr, "Warning: Read error\n");
        return false;
    }
    if (pf.AddToRead(pv[2]) == -1)
    {
        fprintf(stderr, "Warning: Read error\n");
        return false;
    }
...
template <class ScalarType>
bool ScanBBox(const char *fname, Box3<ScalarType> &box, const Matrix44<ScalarType> &m, bool use_cache,
              const char *matrixfname)
{
    if (use_cache)
    {
        if (CheckBBoxCache(fname, box, matrixfname))
            return true;
    }
