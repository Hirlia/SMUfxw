...
void MainWindow::initWidgets()
{
    ...
    ui.actionShow_Noise->setChecked(global_paraMgr.glarea.getBool("Show Noise"));
    ui.actionShow_Normals->setChecked(global_paraMgr.glarea.getBool("Show Normal"));
    ui.actionNeighbor_Normals->setChecked(global_paraMgr.glarea.getBool("Show Neighbor Normal"));
    ...
    ui.actionShow_Face->setChecked(global_paraMgr.glarea.getBool("Show Face"));
    ui.actionshow_ev->setChecked(global_paraMgr.glarea.getBool("Show EV"));
    ui.actionshow_nev->setChecked(global_paraMgr.glarea.getBool("Show NEV"));
    ui.actionneighbor->setChecked(global_paraMgr.glarea.getBool("Show neighbors"));
    ...
    ui.actionShow_Noise_Quads->setChecked(paras->glarea.getBool("Show Noise Quad"));
    ui.actionShow_Noise_Dot->setChecked(paras->glarea.getBool("Show Noise Dot"));
    ui.actionShow_Noise_Circle->setChecked(paras->glarea.getBool("Show Noise Circle"));
    ui.actionShow_Noise_Sphere->setChecked(paras->glarea.getBool("Show Noise Sphere"));
...
void MainWindow::initConnect()
{
    ...
    connect(ui.L0Filter, SIGNAL(triggered()), this, SLOT(showL0filterDlg()));
    connect(ui.actionParticle,SIGNAL(triggered()), this, SLOT(showParticleDlg()));
    connect(ui.actionbilateral,SIGNAL(triggered()),this,SLOT(dobilateral()));
    connect(ui.actionFeature, SIGNAL(triggered()), this, SLOT(calculateFeature()));
    connect(ui.actionwithoutfeature, SIGNAL(triggered()), this, SLOT(withoutshowfeature()));
    connect(ui.actionfeatureplus, SIGNAL(triggered()), this, SLOT(calculateFeatureplus()));
    connect(ui.action_addNoise,SIGNAL(triggered()),SLOT(showNoiseDlg()));
    ...
    connect(ui.actionShow_Noise, SIGNAL(toggled(bool)), this, SLOT(showNoise(bool)));
    connect(ui.actionShow_Normals, SIGNAL(toggled(bool)), this, SLOT(showNormals(bool)));
    connect(ui.actionNeighbor_Normals, SIGNAL(toggled(bool)), this, SLOT(showNeighborNormals(bool)));
    connect(ui.actionShow_Face,SIGNAL(toggled(bool)),this,SLOT(showFaces(bool)));
    connect(ui.actionshow_ev,SIGNAL(toggled(bool)),this,SLOT(showEV(bool)));
    connect(ui.actionshow_nev,SIGNAL(toggled(bool)),this,SLOT(showNEV(bool)));
    connect(ui.actionneighbor,SIGNAL(toggled(bool)),this,SLOT(showneighbors(bool)));
...
void MainWindow::updateStatusBar()
{
    ...
    int n_size = 0;
    if (!area->dataMgr.isNoiseEmpty())
        n_size = area->dataMgr.getCurrentNoise()->vert.size();

//    QString str_nosie = "Noise points: " + QString::number(n_size);
//    noise_size_lable->setText(str_nosie);
...
void MainWindow::showWLopDlg()
{
    ...
    paraDlg_WLOP->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
...
void MainWindow::showNormalDlg()
{
    ...
    paraDlg_Normal->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
...
void MainWindow::showUpsampleDlg()
{
    ...
    paraDlg_Upsample->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
...
void MainWindow::showSkeletonDlg()
{
    ...
    paraDlg_Skeleton->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
...
void MainWindow::showL0filterDlg()
{
    if (paraDlg_L0 != 0)
    {
        paraDlg_L0->close();
    ...
    paraDlg_L0 = new StdParaDlg(paras, area, this);
    paraDlg_L0->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    addDockWidget(Qt::RightDockWidgetArea, paraDlg_L0);
    paraDlg_L0->setFloating(false);
    paraDlg_L0->hide();
    paraDlg_L0->showL0filterParaDlg();
}

void MainWindow::showNoiseDlg()
{
    if (paraDlg_Noise != 0)
    {
        paraDlg_Noise->close();
        delete paraDlg_Noise;
    }

    paraDlg_Noise = new StdParaDlg(paras, area, this);
    paraDlg_Noise->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    addDockWidget(Qt::RightDockWidgetArea, paraDlg_Noise);
    paraDlg_Noise->setFloating(false);
    paraDlg_Noise->hide();
    paraDlg_Noise->showNoiseParaDlg();
}

void MainWindow::showParticleDlg()
{
    if (paraDlg_particle != 0)
    {
        paraDlg_particle->close();
        delete paraDlg_particle;
    }

    paraDlg_particle = new StdParaDlg(paras, area, this);
    paraDlg_particle->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    addDockWidget(Qt::RightDockWidgetArea, paraDlg_particle);
    paraDlg_particle->setFloating(false);
    paraDlg_particle->hide();
    paraDlg_particle->showParticleParaDlg();
}
...
void MainWindow::openFile()
{
    ...
    area->dataMgr.loadPlyToNoise(file);
...
void MainWindow::showNoise(bool _val)
{
    global_paraMgr.glarea.setValue("Show Noise", BoolValue(_val));
    area->updateGL();
}
...
void MainWindow::showNeighborNormals(bool _val)
{
    global_paraMgr.glarea.setValue("Show Neighbor Normal", BoolValue(_val));
    area->updateGL();
}

void MainWindow::showFaces(bool _val)
{
    global_paraMgr.glarea.setValue("Show Face",BoolValue(_val));
    area->updateGL();
}

void MainWindow::showEV(bool _val)
{
    global_paraMgr.glarea.setValue("Show EV",BoolValue(_val));
    area->updateGL();
}

void MainWindow::showNEV(bool _val)
{
    global_paraMgr.glarea.setValue("Show NEV",BoolValue(_val));
    area->updateGL();
}

void MainWindow::showneighbors(bool _val)
{
    global_paraMgr.glarea.setValue("Show neighbors",BoolValue(_val));
    area->updateGL();
}
...
void MainWindow::setNoiseType(QAction *action)
{
    if (action == ui.actionShow_Noise_Quads)
    {
        paras->glarea.setValue("Show Noise Quad", BoolValue(true));
        paras->glarea.setValue("Show Noise Dot", BoolValue(false));
        paras->glarea.setValue("Show Noise Circle", BoolValue(false));
        paras->glarea.setValue("Show Noise Sphere", BoolValue(false));
    }
    else if (action == ui.actionShow_Noise_Dot)
    {
        paras->glarea.setValue("Show Noise Quad", BoolValue(false));
        paras->glarea.setValue("Show Noise Dot", BoolValue(true));
        paras->glarea.setValue("Show Noise Circle", BoolValue(false));
        paras->glarea.setValue("Show Noise Sphere", BoolValue(false));
    }
    else if (action == ui.actionShow_Noise_Circle)
    {
        paras->glarea.setValue("Show Noise Quad", BoolValue(false));
        paras->glarea.setValue("Show Noise Dot", BoolValue(false));
        paras->glarea.setValue("Show Noise Circle", BoolValue(true));
        paras->glarea.setValue("Show Noise Sphere", BoolValue(false));
    }
    else if (action == ui.actionShow_Noise_Sphere)
    {
        paras->glarea.setValue("Show Noise Quad", BoolValue(false));
        paras->glarea.setValue("Show Noise Dot", BoolValue(false));
        paras->glarea.setValue("Show Noise Circle", BoolValue(false));
        paras->glarea.setValue("Show Noise Sphere", BoolValue(true));
    }
    area->updateGL();
}
...
void MainWindow::calculateFeature()
{
    double radius = global_paraMgr.norSmooth.getDouble("CGrid Radius");
    double feat = global_paraMgr.particle.getDouble("feat");
    CMesh *samples = area->dataMgr.getCurrentSamples();
    vcg::NormalExtrapolation<vector<CVertex> >::calculate_pointsigma(samples,samples->vert.begin(), samples->vert.end(),radius,feat);

}

void MainWindow::withoutshowfeature()
{
    CMesh *samples = area->dataMgr.getCurrentSamples();
    vcg::NormalExtrapolation<vector<CVertex> >::withoutshow_feature(samples->vert.begin(), samples->vert.end());
}


void MainWindow::calculateFeatureplus()
{
    //int knn = global_paraMgr.norSmooth.getInt("PCA KNN");
    //double feat = global_paraMgr.norSmooth.getDouble("Feature threshold");
    double sigma_n = global_paraMgr.norSmooth.getDouble("sigma n");
    double radius = global_paraMgr.norSmooth.getDouble("CGrid Radius");
    CMesh *samples = area->dataMgr.getCurrentSamples();
    //vcg::NormalExtrapolation<vector<CVertex> >::calculate_normsigma(samples,samples->vert.begin(), samples->vert.end(),knn,sigma_n);
    //vcg::NormalExtrapolation<vector<CVertex> >::feature_threshod_selection_plus(samples->vert.begin(), samples->vert.end(),feat);
    vcg::NormalExtrapolation<vector<CVertex> >::calculate_feature(samples,samples->vert.begin(),samples->vert.end(),sigma_n,radius);

}

void MainWindow::dobilateral()
{
    global_paraMgr.glarea.setValue("Running Algorithm Name", StringValue("Bilateral"));
    area->runBilateral();
...
void MainWindow::noiseColor()
{
    area->changeColor("Noise Point Color");
    area->updateGL();
}
