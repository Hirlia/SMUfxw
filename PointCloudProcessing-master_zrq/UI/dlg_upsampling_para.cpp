...
bool UpsamplingParaDlg::initWidgets()
{
    ...
    Qt::CheckState state =
      m_paras->upsampling.getBool("Using Threshold Process") ? (Qt::CheckState::Checked) : (Qt::CheckState::Unchecked);
    ui->using_threshol_process->setCheckState(state);

    state = m_paras->upsampling.getBool("Auto Recompute Radius For Dist") ? (Qt::CheckState::Checked) :
