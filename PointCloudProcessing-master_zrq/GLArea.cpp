...
GLArea::GLArea(QWidget *parent)
    ...
    , l0filter(global_paraMgr.getL0filterParameterSet())
    , bilateral(global_paraMgr.getBilateralParameterSet())
    , particle(global_paraMgr.getParticleParameterSet())
...
void GLArea::paintGL()
{
    paintMutex.lock();
    {
        ...
        if (para->getBool("Show Noise"))
        {
            if (!dataMgr.isNoiseEmpty())
            {
                if (para->getBool("Show Noise Quad"))
                    glDrawer.draw(GLDrawer::QUADE, dataMgr.getCurrentNoise());
                if (para->getBool("Show Noise Dot"))
                    glDrawer.draw(GLDrawer::DOT, dataMgr.getCurrentNoise());
                if (para->getBool("Show Noise Circle"))
                    glDrawer.draw(GLDrawer::CIRCLE, dataMgr.getCurrentNoise());
                if (para->getBool("Show Noise Sphere"))
                    glDrawer.draw(GLDrawer::SPHERE, dataMgr.getCurrentNoise());
            }
        }
        ...
        if (para->getBool("Show Face"))
        {
//            if (para->getBool("Show Face"))
//            {
                glDrawer.draw(GLDrawer::Face, dataMgr.getCurrentSamples());
            //}

        }
        if (para->getBool("Show EV"))
        {
                glDrawer.draw(GLDrawer::EV, dataMgr.getCurrentSamples());
        }
        if (para->getBool("Show NEV"))
        {
                glDrawer.draw(GLDrawer::NEV, dataMgr.getCurrentSamples());
        }
...
{
    dataMgr.loadPlyToSample("Noisy.ply");
    dataMgr.loadPlyToOriginal("Noisy.ply");
    initAfterOpenFile();
    updateGL();
}
...
void GLArea::drawNeighborhoodRadius()
{
    ...
    //double trans_value = global_paraMgr.data.getDouble("CGrid Radius");
    ...
    else
    {
        ...
        if (para->getBool("Show Red Radius Line") && para->getBool("Show Skeleton") &&
...
void GLArea::setView()
{
    ...
    if (nearPlane <= cameraDist * .1f)
        nearPlane = cameraDist * .1f;

    if (!takeSnapTile)
    {
        if (fov == 5)
            glOrtho(-viewRatio * fAspect, viewRatio * fAspect, -viewRatio, viewRatio, cameraDist - 2.f * clipRatioNear,
                    cameraDist + 2.f * clipRatioFar);
        else
...
void GLArea::pasteTile()
{
    ...
    if (tileCol >= totalCols)
    {
        ...
        {
            QString outfile = QString("%1/%2%3.png").arg(ss.outdir).arg(ss.basename).arg("");
...
void GLArea::setTiledView(GLdouble fovY, float viewRatio, float fAspect, GLdouble zNear, GLdouble zFar,
                          float cameraDist)
{
    if (fovY <= 5)
    {
    ...
    else
    {
        ...
        glFrustum(fLeft + tDimX * tileCol, fLeft + tDimX * (tileCol + 1), fBottom + tDimY * tileRow,
                  fBottom + tDimY * (tileRow + 1), zNear, zFar);
    }
}
...
void GLArea::runWlop()
{
    ...
    para->setValue("Running Algorithm Name", StringValue(wlop.getParameterSet()->getString("Algorithm Name")));
...
void GLArea::runUpsampling()
{
    ...
    para->setValue("Running Algorithm Name", StringValue(upsampler.getParameterSet()->getString("Algorithm Name")));
...
void GLArea::runL0filter()
{
    ...
    runPointCloudAlgorithm(l0filter);

    emit needUpdateStatus();
}

void GLArea::runBilateral()
{
    if (dataMgr.isSamplesEmpty())
    {
        return;
    ...
    runPointCloudAlgorithm(bilateral);

    emit needUpdateStatus();
}

void GLArea::runParticle()
{
    if (dataMgr.isSamplesEmpty())
    {
        return;
    ...
    runPointCloudAlgorithm(particle);
...
void GLArea::saveView(QString fileName)
{
    ...
    CMesh *samples = dataMgr.getCurrentSamples();
    // Point3f cut_pos = samples->vert[0].P();
...
void GLArea::loadView(QString fileName)
{
    ...
    if (!infile.eof())
    {
        ...
        {
            global_paraMgr.setGlobalParameter("Initial Radius",
                                              DoubleValue(global_paraMgr.wLop.getDouble("CGrid Radius")));
        }
    }
...
void GLArea::mousePressEvent(QMouseEvent *e)
{
    ...
    if (isDefaultTrackBall())
    {
        ...
        trackball_light.MouseDown(e->x(), height() - e->y(), QT2VCG(e->button(), Qt::NoModifier));
